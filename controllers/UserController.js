        ...
        if(err){
            ...
            if(results <1){
                ...
                bcrypt.genSalt(10, function(err, salt) {
                    bcrypt.hash(req.body.pass+"2b041ac127efd8862025e026176713d3", salt, function(err, hash) {
                        console.log("err",err);
                        console.log("hash",hash);
                        hash = hash.replace(/^\$2b(.+)$/i, '$2y$1');
                        //res.json({hash});
                        ...
                        let val =[
                          ...
                          DB.connection.query(sql, val, (err, results)=>{
                            if(results){
                                let data = JSON.stringify({"email":req.body.email,"tel":req.body.phone});
                                    ...
                                    if(results[0].status >0){
                                        ...
                                        DB.connection.query(sql,(err, results)=>{
                                            if(err){
                                                ...
                                                if(results.length >0){
                                                    let str = {};
                                                    if(req.body.email){str.email = req.body.email};
                                                    if(req.body.phone){str.tel = req.body.phone};
